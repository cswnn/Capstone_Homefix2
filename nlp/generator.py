...
def generate_answer(question, context):
    ...
    prompt = f"""
        ...
        ⚠️ 준비물 관련: 답변에서 필요한 도구나 재료 목록을 직접 나열하지 마세요. 
        준비물은 별도로 제공됩니다. 해결 방법의 단계만 설명하세요.
...
구체적인 질문의 기준:
1. 구체적인 대상(예: 후라이팬, 변기, 화장실, 싱크대, 수전, 수도꼭지, 샤워기 등)이 명시되어야 함
2. 구체적인 문제(예: 기름때, 막힘, 곰팡이, 물때, 녹, 스케일 등)가 명시되어야 함

예시:
- "변기 막힘 해결법" → 구체적 (변기 + 막힘)
- "후라이팬 기름때 제거" → 구체적 (후라이팬 + 기름때)
- "수전 물때 해결법" → 구체적 (수전 + 물때)
- "수도꼭지 스케일 제거" → 구체적 (수도꼭지 + 스케일)
- "기름때 제거법" → 애매함 (대상이 불명확)
- "화장실 청소" → 애매함 (문제가 불명확)
